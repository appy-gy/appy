...
server.on 'request', (req, res) ->
  target = if (req.method == 'GET' and req.url.match(/^\/api\/private\/(.*?)$/)) or req.url.match(/^\/api\/private\/(ratings\/[^\/]+\/(rating_items(.*?)|view|likes|tags|comments)|users\/(.+)|rating_items(.*?)|client_errors)$/)
